...
SELECT student.id, student.name FROM student INNER JOIN payment ON student.id = payment.student_id GROUP BY student.name HAVING COUNT(payment_date) > 2 AND YEAR(payment_date) = 2019;